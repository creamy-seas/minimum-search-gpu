...
class TestPotentialFunction(unittest.TestCase):
    def setUp(self):
        ...
        def kernel_wrapped_func(
                phi01: float, phi02: float, phi03: float, L: float, R: float, alpha: float,
                store_value
        ):
            store_value[0] = potential_function_cuda(
    ...
    def test_potential_function_cuda(self):
        store_result = cuda.device_array(
            shape=(1),
            dtype=np.float32
        )

        test_result = self.sut[1, 1](
            0, 0, 0, 0, 0, 1,
            store_result
        )
